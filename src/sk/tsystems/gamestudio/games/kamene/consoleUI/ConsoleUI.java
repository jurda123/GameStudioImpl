package sk.tsystems.gamestudio.games.kamene.consoleUI;
...
import sk.tsystems.gamestudio.games.kamene.Core.Field;
...
//		}
	...
	public void update() {
		System.out.println("Time: " + getPlayingTime() + " s");
	...
	private void move(int dx, int dy) {
		...
		if (canBeMovedX(x1, dx) && canBeMovedY(y1, dy)) {
			field.swap(x1, y1, x2, y2);
		}

	}

	private boolean canBeMovedX(int x, int dx) {
		if (x + dx < 0) {
			return false;
		}
	...
	private boolean canBeMovedY(int y, int dy) {
		if (y + dy < 0) {
			return false;
		}

		else if (y + dy >= field.getColumnCount()) {
			return false;
		}
